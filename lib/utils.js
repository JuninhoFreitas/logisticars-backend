...
export async function checkToken(token) {
  ...
  let session = await mainDB.session.findFirst({ where: { token } });
  if (session) {
      let user = await mainDB.usuario.findFirst({ where: { token: session.user_ref }, select: { id: true, username: true, token: true } });
      session.user = user;
